...
def run_subj_pred_gender_numerals(
        sentences: list[conllu.TokenList],
        morph_dict: MorphDictionary,
        limit: Optional[int],
) -> pd.DataFrame:
    def transform(sentence: conllu.TokenList) -> bool:
        matches = extract_subj_pred_gender_numerals(sentence)
    ...
    return run_filter_transform(
        sentences,
        match_subj_pred_gender_numerals,
        transform,
        limit=limit,
        progress_desc="subj_pred_gender_numerals",
    )


def match_subj_pred_gender_numerals(
        sentence: conllu.TokenList,
) -> bool:
    return extract_subj_pred_gender_numerals(sentence) is not None


def extract_subj_pred_gender_numerals(
        sentence: conllu.TokenList,
