...
from phenomena.common import run_filter_transform, change_gender
from phenomena.morph_dictionary import MorphDictionary


def run_subj_pred_gender_genitive(
        sentences: list[conllu.TokenList],
        morph_dict: MorphDictionary,
        limit: Optional[int],
) -> pd.DataFrame:
    def transform(sentence: conllu.TokenList) -> bool:
        matches = extract_subj_pred_gender_genitive(sentence)
    ...
    return run_filter_transform(
        sentences,
        match_subj_pred_gender_genitive,
        transform,
        limit=limit,
        progress_desc="subj_pred_gender_genitive",
    )


def match_subj_pred_gender_genitive(
        sentence: conllu.TokenList,
) -> bool:
    return extract_subj_pred_gender_genitive(sentence) is not None


def extract_subj_pred_gender_genitive(
        sentence: conllu.TokenList,
    ...
    for child in root.children:
        ...
        if child.token.get("lemma") in {"to", "by"}:
            continue
        if child_feats.get("Gender") != "Neut":
            continue
        cop = child
        break
